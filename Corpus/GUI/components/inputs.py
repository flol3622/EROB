...
def add_input_field(layout, app_instance, label_text, tooltip, placeholder, attribute_name=None):
    row_layout = QHBoxLayout()
    label = QLabel(label_text)
    input_field = QLineEdit()
    if attribute_name is not None:
        setattr(app_instance, attribute_name, input_field)
...
def add_spinbox(layout, app_instance, label_text, tooltip, min_val, max_val, default_val, attribute_name=None):
    row_layout = QHBoxLayout()
    label = QLabel(label_text)
    spinbox = QSpinBox()
    ...
    if attribute_name is not None:
        setattr(app_instance, attribute_name, spinbox)
    info_button = QPushButton('?')
    info_button.setFixedWidth(20)
    info_button.clicked.connect(lambda: QMessageBox.information(layout.parentWidget(), 'Information', tooltip))
    row_layout.addWidget(label)
...
def add_combobox(layout, app_instance, label_text, tooltip, options, default_val, attribute_name=None):
    row_layout = QHBoxLayout()
    label = QLabel(label_text)
    combobox = QComboBox()
    ...
    if attribute_name is not None:
        setattr(app_instance, attribute_name, combobox)
    info_button = QPushButton('?')
    info_button.setFixedWidth(20)
    info_button.clicked.connect(lambda: QMessageBox.information(layout.parentWidget(), 'Information', tooltip))
    row_layout.addWidget(label)
